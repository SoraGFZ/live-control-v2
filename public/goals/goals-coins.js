...
let settings = {
            ...
            try {
                ...
                if(data.ok && data.config) {
                    ...
                    if(data.accumulator && (data.accumulator.coins > 0 || data.accumulator.isActive)) {
                        ...
                        settings.current = 0;
                        // console.log('[Goals Widget - Coins] ?? Iniciando con current=0 (acumulador vacío)');
                    }
        ...
        function animateProgress(targetPercentage) {
            ...
            if(diff < 2) {
                ...
                // console.log('[Goals Widget - Coins] ? Actualización INSTANTÁNEA a', currentProgress.toFixed(2) + '%');
                return;
            }
        ...
        async function connectToServer() {
            ...
            if (__TC_IS_HTTPS_PAGE) {
                ...
                console.warn('[Goals Widget - Coins] Página servida en HTTPS sin uid: no se puede conectar al servidor local.');
                return;
            }
            ...
            if(typeof io !== 'undefined') {
                ...
                socket.on('connect_error', (error) => {
                    console.error('[Goals Widget - Coins] ? Error de conexión Socket.IO:', error);
                });

                // ?? Escuchar eventos TikTok crudos SOLO para el avatar flip
                // (decorativo). El valor del goal lo sigue actualizando el
                // acumulador. Sin esto, el flip nunca dispararía en local.
                ...
                socket.on('goals:accumulator:update', (data) => {
                    ...
                    try {
                        ...
                        console.error('[Goals Widget - Coins] ? Error procesando actualización del acumulador:', e);
                    }
                });
            } else {
                console.error('[Goals Widget - Coins] ? Socket.IO no disponible después de cargar el script');
            }
        }
        ...
        async function saveGoalToServer() {
            try {
                // ? ENVIAR SOLO LAS KEYS DE ESTA MÉTRICA (coins), no todo el config
                const partialConfig = {
                ...
                // console.log('[Goals Widget - coins] ?? Guardando SOLO esta métrica (coins) al servidor');
        ...
        async function initializeWithRetry() {
            ...
            if(success) {
                ...
                console.warn('[Goals Widget - Coins] ?? No se pudo cargar la configuración después de', maxAttempts, 'intentos.');
                applySettings();
                connectToServer();
            }
        }
