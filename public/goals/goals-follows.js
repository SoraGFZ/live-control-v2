...
const __TC_GOALS_WIDGET = 'goals-follows';
...
let settings = {
        ...
        async function loadConfig(isInitialLoad = false) {
            try {
                ...
                if(data.ok && data.config) {
                    ...
                    // Verificar que tiene datos de Follows
                    const hasConfig = parsed.goalfollows_title !== undefined || parsed.goalfollows_value !== undefined;
                    if(!hasConfig && isInitialLoad) {
                        // [log cleaned]
                        return false;
                    }
                    
                    settings.title = parsed.goalfollows_title || 'follows';
                    settings.target = parsed.goalfollows_value || 100;
                    settings.originalTarget = parsed.goalfollows_originalValue || parsed.goalfollows_value || 100;
                    settings.progress1Color = parsed.goalfollows_progress1Color || '#00d9ff';
                    settings.progress2Color = parsed.goalfollows_progress2Color || '#2cb2d4';
                    settings.styleImage = parsed.goalfollows_styleImage || '';
                    settings.whenReached = parsed.goalfollows_whenReached || 'keep';
                    settings.textColor = parsed.goalfollows_textColor || '#ffffff';
                    settings.textSize = parsed.goalfollows_textSize || 20;
                    settings.textBorderColor = parsed.goalfollows_textBorderColor || '#000000';
                    settings.textBorderWidth = parsed.goalfollows_textBorderWidth || 0;
                    settings.rainbowEnabled = parsed.goalfollows_rainbowEnabled || false;
                    settings.patternEnabled = parsed.goalfollows_patternEnabled || false;
                    settings.patternUrl = parsed.goalfollows_patternUrl || '';
                    settings.patternOpacity = parsed.goalfollows_patternOpacity !== undefined ? parsed.goalfollows_patternOpacity : 0.5;
                    settings.fontFamily = parsed.goalfollows_fontFamily || 'System Default';
                    settings.fontUrl = parsed.goalfollows_fontUrl || '';
                    settings.reachAnimation = parsed.goalfollows_reachAnimation || 'shake';
                    settings.widgetType = parsed.goalfollows_widgetType || 'bar';
                    settings.compactTransparentBg = !!parsed.goalfollows_compactTransparentBg; // 'bar' | 'compact'
                    settings.changeAnimation = parsed.goalfollows_changeAnimation || 'flip';
                    // Modo radial: avatar flip
                    settings.avatarFlipEnabled = parsed.goalfollows_avatarFlipEnabled !== false;
                    settings.avatarFlipEvery = parseInt(parsed.goalfollows_avatarFlipEvery, 10) || 1;
                    settings.compactLabelColor = parsed.goalfollows_compactLabelColor || '';
                    settings.compactTargetColor = parsed.goalfollows_compactTargetColor || '';
                    settings.disableAbbreviation = !!parsed.goalfollows_disableAbbreviation;
                    settings._timestamp = parsed._timestamp || 0;
                    
                    // ? CARGAR DESDE EL ACUMULADOR TEMPORAL (persiste entre recargas de la URL)
                    // ? PRIORIDAD 1: Si hay datos en el acumulador (incluso si está inactivo), usarlos
                    if(data.accumulator && (data.accumulator.follows > 0 || data.accumulator.isActive)) {
                        settings.current = data.accumulator.follows || 0;
                        // console.log('[Goals Widget - follows] ? Cargado desde acumulador temporal:', settings.current, '(isActive:', data.accumulator.isActive, ')');
                    } else if(isInitialLoad) {
                        settings.current = 0;
                        // console.log('[Goals Widget - follows] ?? Iniciando con current=0 (acumulador vacío)');
                    }
                ...
                console.error('[Goals Widget - follows] Error cargando config:', e);
                return false;
            }
        }
        ...
        function applySettings() {
            ...
            if(!barFill || !barBg || !textEl || !overlay || !pattern) {
                console.error('[Goals Widget - follows] ? No se encontraron elementos del DOM');
                return;
            }
            ...
            // Aplicar patrón de imagen/GIF (prioridad alta)
            if(settings.patternEnabled && settings.patternUrl) {
                ...
                barFill.style.backgroundColor = settings.progress1Color || '#00d9ff';
        ...
        function updateDisplay() {
            ...
            const textEl = document.getElementById('bar-text');
            if(textEl) {
                textEl.textContent = `${settings.title.toUpperCase()} - ${settings.current} / ${settings.target} follows`;
            }
        ...
        function animateProgress(targetPercentage) {
            ...
            // console.log('[Goals Widget - follows] ?? animateProgress: currentProgress=' + currentProgress.toFixed(2) + '% ? target=' + targetPercentage.toFixed(2) + '% (diff=' + diff.toFixed(2) + '%)');
            
            if(diff < 2) {
                currentProgress = targetPercentage;
                document.getElementById('bar-fill').style.width = currentProgress + '%';
                // console.log('[Goals Widget - follows] ? Actualización INSTANTÁNEA a', currentProgress.toFixed(2) + '%');
                return;
            }
        ...
        async function connectToServer() {
            ...
            if (__TC_IS_HTTPS_PAGE) {
                ...
                console.warn('[Goals Widget - follows] Página servida en HTTPS sin uid: no se puede conectar al servidor local.');
                return;
            }
            ...
            await new Promise((resolve) => {
                ...
                script.onerror = (error) => {
                    console.error('[Goals Widget - follows] ? Error cargando Socket.IO script:', error);
                    resolve();
                };
            ...
            if(typeof io !== 'undefined') {
                ...
                socket.on('connect_error', (error) => {
                    console.error('[Goals Widget - follows] ? Error de conexión Socket.IO:', error);
                });

                // ?? Eventos TikTok crudos SOLO para el avatar flip
                socket.on('tiktok:event', (evt) => {
                    if (evt && evt.type === 'follow') maybeFlipAvatar(evt.data || evt);
                });
                ...
                socket.on('goals:accumulator:update', (data) => {
                    // [log cleaned]
                    try {
                        if(data.metric === 'follows' && data.current !== undefined) {
                            const oldCurrent = settings.current;
                            settings.current = data.current;
                            
                            // [log cleaned]
                            // Ajustar goal automáticamente si se superó
                            let timesReached = 0;
                            while(settings.current >= settings.target && timesReached < 1000) {
                                timesReached++;
                                handleGoalReached();
                        ...
                        console.error('[Goals Widget - follows] ? Error procesando actualización del acumulador:', e);
                    }
                });
            } else {
                console.error('[Goals Widget - follows] ? Socket.IO no disponible después de cargar el script');
            }
        }
        ...
        function handleGoalReached() {
            ...
            // Aplicar animación al alcanzar objetivo
            ...
            // Aplicar acción configurada con animación al cambiar
        ...
        async function saveGoalToServer() {
            try {
                // ? ENVIAR SOLO LAS KEYS DE ESTA MÉTRICA (follows), no todo el config
                const partialConfig = {
                    goalfollows_value: settings.target,
                    goalfollows_title: settings.title || 'FOLLOWS',
                    goalfollows_whenReached: settings.whenReached || 'nothing',
                    goalfollows_action: settings.action || {},
                    goalfollows_styles: settings.styles || [],
                    goalfollows_currentStyleIndex: settings.currentStyleIndex || 0,
                    _timestamp: Date.now()
                };

                // ?? NO guardar el "current"
                // console.log('[Goals Widget - follows] ?? Guardando SOLO esta métrica (follows) al servidor');
                ...
                console.error('[Goals Widget - follows] ? Error guardando goal:', e);
            }
        }
        ...
        async function initializeWithRetry() {
            ...
            if(success) {
                ...
                // console.log(`[Goals Widget - follows] Reintentando carga (${loadAttempts}/${maxAttempts})...`);
                setTimeout(initializeWithRetry, 200);
            } else {
                console.warn('[Goals Widget - follows] No se pudo cargar la configuracion despues de', maxAttempts, 'intentos.');
                applySettings();
                connectToServer();
            }
        }
        ...
        // Recargar configuración cuando detecte cambios (polling cada 2 segundos)
        setInterval(async () => {
            try {
if (__TC_IS_CLOUD) return; // cloud usa push (widget:configUpdated)
                ...
                if(data.ok && data.config) {
                    ...
                    if(newTimestamp > lastConfigTimestamp) {
                        ...
                        loadConfig(false);
                        // console.log('[Goals Widget - follows] ?? Config recargada por cambio en archivo (timestamp: ' + newTimestamp + ')');
                    }
                }
            } catch(e) {
        ...
        function applyCompactSettings() {
            ...
            if (labelEl) labelEl.textContent = (settings.title || 'follows').toUpperCase();
