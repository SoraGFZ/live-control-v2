...
const __TC_GOALS_WIDGET = 'goals-shares';
...
let settings = {
        ...
        async function loadConfig(isInitialLoad = false) {
            try {
                ...
                if(data.ok && data.config) {
                    ...
                    const hasConfig = parsed.goalshares_title !== undefined || parsed.goalshares_value !== undefined;
                    if(!hasConfig && isInitialLoad) {
                        return false;
                    }
                    
                    settings.title = parsed.goalshares_title || 'COMPARTIDOS';
                    settings.target = parsed.goalshares_value || 50;
                    settings.originalTarget = parsed.goalshares_originalValue || parsed.goalshares_value || 50;
                    settings.progress1Color = parsed.goalshares_progress1Color || '#2ecc71';
                    settings.progress2Color = parsed.goalshares_progress2Color || '#2cb2d4';
                    settings.styleImage = parsed.goalshares_styleImage || '';
                    settings.whenReached = parsed.goalshares_whenReached || 'keep';
                    settings.textColor = parsed.goalshares_textColor || '#ffffff';
                    settings.textSize = parsed.goalshares_textSize || 20;
                    settings.textBorderColor = parsed.goalshares_textBorderColor || '#000000';
                    settings.textBorderWidth = parsed.goalshares_textBorderWidth || 0;
                    settings.rainbowEnabled = parsed.goalshares_rainbowEnabled || false;
                    settings.patternEnabled = parsed.goalshares_patternEnabled || false;
                    settings.patternUrl = parsed.goalshares_patternUrl || '';
                    settings.patternOpacity = parsed.goalshares_patternOpacity !== undefined ? parsed.goalshares_patternOpacity : 0.5;
                    settings.fontFamily = parsed.goalshares_fontFamily || 'System Default';
                    settings.fontUrl = parsed.goalshares_fontUrl || '';
                    settings.reachAnimation = parsed.goalshares_reachAnimation || 'shake';
                    settings.widgetType = parsed.goalshares_widgetType || 'bar';
                    settings.compactTransparentBg = !!parsed.goalshares_compactTransparentBg; // 'bar' | 'compact'
                    settings.changeAnimation = parsed.goalshares_changeAnimation || 'flip';
                    // Modo radial: avatar flip (en shares cuenta por usuario)
                    settings.avatarFlipEnabled = parsed.goalshares_avatarFlipEnabled !== false;
                    settings.avatarFlipEvery = parseInt(parsed.goalshares_avatarFlipEvery, 10) || 10;
                    settings.compactLabelColor = parsed.goalshares_compactLabelColor || '';
                    settings.compactTargetColor = parsed.goalshares_compactTargetColor || '';
                    settings.disableAbbreviation = !!parsed.goalshares_disableAbbreviation;
                    settings._timestamp = parsed._timestamp || 0;
                    
                    if(data.accumulator && (data.accumulator.shares > 0 || data.accumulator.isActive)) {
                        settings.current = data.accumulator.shares || 0;
                ...
                console.error('[Goals Widget - shares] Error cargando config:', e);
                return false;
            }
        }
        ...
        function applySettings() {
            ...
            if(!barFill || !barBg || !textEl || !overlay || !pattern) {
                console.error('[Goals Widget - shares] No se encontraron elementos del DOM');
                return;
            }
            ...
            if(settings.patternEnabled && settings.patternUrl) {
                ...
                barFill.style.backgroundColor = settings.progress1Color || '#2ecc71';
        ...
        async function connectToServer() {
            if (__TC_IS_CLOUD) {
                __tcOnCloudEvent((evt) => {
                    if (evt && evt.type === 'share') {
                        applyRealtimeIncrement(1, evt.data || evt, { perUser: true });
                    }
                });
                return;
            }
            if (__TC_IS_HTTPS_PAGE) {
                ...
                console.warn('[Goals Widget - shares] Página servida en HTTPS sin uid: no se puede conectar al servidor local.');
                return;
            }
            ...
            if(typeof io !== 'undefined') {
                ...
                socket.on('connect_error', (error) => {
                    console.error('[Goals Widget - shares] Error de conexión Socket.IO:', error);
                });

                socket.on('tiktok:event', (evt) => {
                    if (evt && evt.type === 'share') maybeFlipAvatar(evt.data || evt, { perUser: true });
                });

                socket.on('goals:accumulator:update', (data) => {
                    try {
                        if(data.metric === 'shares' && data.current !== undefined) {
                        ...
                        console.error('[Goals Widget - shares] Error procesando actualización:', e);
                    }
                });
            }
        }
        ...
        async function saveGoalToServer() {
            try {
                const partialConfig = {
                    goalshares_value: settings.target,
                    goalshares_title: settings.title || 'COMPARTIDOS',
                    goalshares_whenReached: settings.whenReached || 'nothing',
                    _timestamp: Date.now()
                };
                ...
                console.error('[Goals Widget - shares] Error guardando goal:', e);
            }
        }
...
if (__TC_IS_CLOUD) return; // cloud usa push (widget:configUpdated)
        ...
        function applyCompactSettings() {
            ...
            if (labelEl) labelEl.textContent = (settings.title || 'shares').toUpperCase();
