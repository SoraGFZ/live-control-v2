...
const __TC_GOALS_WIDGET = 'goals-subscribers';
...
let settings = {
        ...
        async function loadConfig(isInitialLoad = false) {
            try {
                ...
                if(data.ok && data.config) {
                    ...
                    settings.title = parsed.goalsubscribers_title || 'subscribers';
                    settings.target = parsed.goalsubscribers_value || 10;
                    settings.originalTarget = parsed.goalsubscribers_originalValue || parsed.goalsubscribers_value || 10;
                    settings.progress1Color = parsed.goalsubscribers_progress1Color || '#9b59b6';
                    settings.progress2Color = parsed.goalsubscribers_progress2Color || '#2cb2d4';
                    settings.styleImage = parsed.goalsubscribers_styleImage || '';
                    settings.whenReached = parsed.goalsubscribers_whenReached || 'keep';
                    settings.textColor = parsed.goalsubscribers_textColor || '#ffffff';
                    settings.textSize = parsed.goalsubscribers_textSize || 20;
                    settings.textBorderColor = parsed.goalsubscribers_textBorderColor || '#000000';
                    settings.textBorderWidth = parsed.goalsubscribers_textBorderWidth || 0;
                    settings.rainbowEnabled = parsed.goalsubscribers_rainbowEnabled || false;
                    settings.patternEnabled = parsed.goalsubscribers_patternEnabled || false;
                    settings.patternUrl = parsed.goalsubscribers_patternUrl || '';
                    settings.patternOpacity = parsed.goalsubscribers_patternOpacity !== undefined ? parsed.goalsubscribers_patternOpacity : 0.5;
                    settings.fontFamily = parsed.goalsubscribers_fontFamily || 'System Default';
                    settings.fontUrl = parsed.goalsubscribers_fontUrl || '';
                    settings.reachAnimation = parsed.goalsubscribers_reachAnimation || 'shake';
                    settings.widgetType = parsed.goalsubscribers_widgetType || 'bar';
                    settings.compactTransparentBg = !!parsed.goalsubscribers_compactTransparentBg; // 'bar' | 'compact'
                    settings.changeAnimation = parsed.goalsubscribers_changeAnimation || 'flip';
                    // Modo radial: avatar flip
                    settings.avatarFlipEnabled = parsed.goalsubscribers_avatarFlipEnabled !== false;
                    settings.avatarFlipEvery = parseInt(parsed.goalsubscribers_avatarFlipEvery, 10) || 1;
                    settings.compactLabelColor = parsed.goalsubscribers_compactLabelColor || '';
                    settings.compactTargetColor = parsed.goalsubscribers_compactTargetColor || '';
                    settings.disableAbbreviation = !!parsed.goalsubscribers_disableAbbreviation;
                    ...
                    if(data.accumulator && (data.accumulator.subscribers > 0 || data.accumulator.isActive)) {
                        settings.current = data.accumulator.subscribers || 0;
                        // console.log('[Goals Widget - subscribers] ? Cargado desde acumulador temporal:', settings.current, '(isActive:', data.accumulator.isActive, ')');
                    } else if(isInitialLoad) {
                        settings.current = 0;
                        // console.log('[Goals Widget - subscribers] ?? Iniciando con current=0 (acumulador vacío)');
                    }
                ...
                console.error('[Goals Widget - subscribers] ? Error cargando config:', e);
            }
        }
        ...
        function applySettings() {
            ...
            if(!barFill || !barBg || !textEl || !overlay || !pattern) {
                console.error('[Goals Widget - subscribers] ? No se encontraron elementos del DOM');
                return;
            }
            ...
            if(settings.patternEnabled && settings.patternUrl) {
                ...
                barFill.style.backgroundColor = settings.progress1Color || '#9b59b6';
        ...
        function updateDisplay() {
            ...
            const textEl = document.getElementById('bar-text');
            if(textEl) {
                textEl.textContent = `${settings.title.toUpperCase()} - ${settings.current} / ${settings.target} subscribers`;
            }
        ...
        function animateProgress(targetPercentage) {
            ...
            // console.log('[Goals Widget - subscribers] ?? animateProgress: currentProgress=' + currentProgress.toFixed(2) + '% ? target=' + targetPercentage.toFixed(2) + '% (diff=' + diff.toFixed(2) + '%)');
            
            if(diff < 2) {
                currentProgress = targetPercentage;
                document.getElementById('bar-fill').style.width = currentProgress + '%';
                // console.log('[Goals Widget - subscribers] ? Actualización INSTANTÁNEA a', currentProgress.toFixed(2) + '%');
                return;
            }
        ...
        async function connectToServer() {
            if (__TC_IS_CLOUD) {
                __tcOnCloudEvent((evt) => {
                    if (!evt) return;
                    if (evt.type === 'subscribe' || evt.type === 'superFan') {
            ...
            if (__TC_IS_HTTPS_PAGE) {
                ...
                console.warn('[Goals Widget - subscribers] Página servida en HTTPS sin uid: no se puede conectar al servidor local.');
                return;
            }
            ...
            await new Promise((resolve) => {
                ...
                script.onerror = (error) => {
                    console.error('[Goals Widget - subscribers] ? Error cargando Socket.IO script:', error);
                    resolve();
                };
            ...
            if(typeof io !== 'undefined') {
                ...
                socket.on('connect_error', (error) => {
                    console.error('[Goals Widget - subscribers] ? Error de conexión Socket.IO:', error);
                });
                ...
                socket.on('goals:accumulator:update', (data) => {
                    // [log cleaned]
                    try {
                        if(data.metric === 'subscribers' && data.current !== undefined) {
                        ...
                        console.error('[Goals Widget - subscribers] ? Error procesando actualización del acumulador:', e);
                    }
                });
            } else {
                console.error('[Goals Widget - subscribers] ? Socket.IO no disponible después de cargar el script');
            }
        }
        ...
        async function saveGoalToServer() {
            try {
                // ? ENVIAR SOLO LAS KEYS DE ESTA MÉTRICA (subscribers), no todo el config
                const partialConfig = {
                    goalsubscribers_value: settings.target,
                    goalsubscribers_title: settings.title || 'SUPERFANS',
                    goalsubscribers_whenReached: settings.whenReached || 'nothing',
                    goalsubscribers_action: settings.action || {},
                    goalsubscribers_styles: settings.styles || [],
                    goalsubscribers_currentStyleIndex: settings.currentStyleIndex || 0,
                    _timestamp: Date.now()
                };

                // ?? NO guardar el "current"
                // console.log('[Goals Widget - subscribers] ?? Guardando SOLO esta métrica (subscribers) al servidor');
                ...
                console.error('[Goals Widget - subscribers] ? Error guardando goal:', e);
            }
        }
...
if (__TC_IS_CLOUD) return; // cloud usa push (widget:configUpdated)
                ...
                if(data.ok && data.config) {
                    ...
                    if(newTimestamp > lastConfigTimestamp) {
                        ...
                        loadConfig(false);
                        // console.log('[Goals Widget - subscribers] ?? Config recargada por cambio en archivo (timestamp: ' + newTimestamp + ')');
                    }
                }
            } catch(e) {
        ...
        function applyCompactSettings() {
            ...
            if (labelEl) labelEl.textContent = (settings.title || 'subscribers').toUpperCase();
