...
// no hay 403 ni mixed-content. En localhost (OBS misma máquina) se
...
function __tcCloudConnect() {
    ...
    try {
        ...
        __tcCloudWs.onerror = () => { /* onclose maneja la reconexión */ };
    } catch (e) {
...
let settings = {
        ...
        let updateThrottle = null; // Para agrupar actualizaciones rápidas
        ...
        // Cargar configuración desde el servidor (cloud o localhost)
        async function loadConfig(isInitialLoad = false) {
            try {
                ...
                if(data.ok && data.config) {
                    ...
                    // Colores específicos del modo radial (label, value y target)
                    ...
                    applySettings();
                    return true; // Indicar que la config se cargó correctamente
                }
        ...
        function applySettings() {
            ...
            // pega siempre goals-likes.html y el modo se decide aquí.
            ...
            if(settings.patternEnabled && settings.patternUrl) {
                ...
                barFill.style.backgroundSize = '50px 50px'; // Tamaño fijo para repetirse
                barFill.style.backgroundPosition = '0 0';
                barFill.style.backgroundRepeat = 'repeat'; // Repetir patrón
            ...
            //   widget corre en localhost; si está en URL pública (HTTPS),
            //   se remapean al CDN público para evitar Mixed Content.
        ...
        // Actualizar display (con throttling para actualizaciones rápidas)
        function updateDisplay() {
            ...
            // Actualizar inmediatamente sin throttling para máxima velocidad
        ...
        function applyCompactSettings() {
            ...
            // El % de la línea target hereda el fill-color, lo respetamos.
        ...
        function formatCompactCount(n) {
            // Toggle "Mostrar números completos" (settings.disableAbbreviation):
            // si está activo, muestra el valor con separadores de miles (ej.
        ...
        // el avatar del último usuario que disparó el evento durante 3.5s,
        // luego vuelve al SVG. Para shares cuenta por uniqueId; el resto
        // suma global. Implementación compartida entre los 6 widgets goals.
        ...
        function triggerAvatarFlip(avatarUrl) {
            ...
            // Si está en mitad del flip, solo actualiza la imagen y resetea el timer
            img.src = avatarUrl;
            if (__avatarBusy) {
        ...
        // Animar progreso (ultra rápido - actualización casi instantánea)
        function animateProgress(targetPercentage) {
            ...
            // Si la diferencia es muy pequeña (<2%), actualizar INSTANTÁNEAMENTE
            if(diff < 2) {
                currentProgress = targetPercentage;
                document.getElementById('bar-fill').style.width = currentProgress + '%';
                // console.log('[Goals Widget] ? Actualización INSTANTÁNEA a', currentProgress.toFixed(2) + '%');
                return;
            }

            // Para diferencias pequeñas (2-10%), animar muy rápido
            if(diff < 10) {
            ...
            // Para diferencias grandes, animar rápido pero suave
            const steps = Math.min(8, Math.ceil(diff / 5)); // Máximo 8 pasos
            const increment = (targetPercentage - currentProgress) / steps;
            ...
            progressInterval = setInterval(() => {
                ...
                document.getElementById('bar-fill').style.width = currentProgress + '%';
            }, 10); // 10ms por paso (máximo 80ms para diferencias grandes)
        }

        // Procesa un evento TikTok (común para cloud y local)
        function processTikTokEvent(ev) {
            try {
                ...
                if (ev.type === 'like') {
                    ...
                    // siguiente 137808 (de otro shard que iba atrás). Si lo
                    // copiamos directo, el contador rebota visualmente.
                    // Solución: solo aceptar totalLikeCount si AVANZA.
                    ...
                    if (reportedTotal != null && reportedTotal > currentVal) {
                        ...
                        // El total reportado es viejo (o no llegó). Usar el
                    ...
                    // Si la meta se alcanzó al menos una vez, animar y guardar
                    // UNA SOLA VEZ — sin importar si el while iteró 1 o 200
                    // veces. Antes cada iteración disparaba animación + save
                    // ? spam al worker + oscilación visual.
                    if (timesReached > 0) {
        ...
        async function connectToServer() {
            ...
            if (__TC_IS_HTTPS_PAGE) {
                ...
                console.warn('[Goals Widget] Página servida en HTTPS sin uid: no se puede conectar al servidor local.');
                return;
            }
            ...
            if (typeof io !== 'undefined') {
                ...
                socket.on('connect_error', (error) => {
                    console.error('[Goals Widget] ? Error de conexión Socket.IO:', error);
                });
                socket.on('tiktok:event', processTikTokEvent);
            } else {
                console.error('[Goals Widget] ? Socket.IO no disponible después de cargar el script');
            }
        }

        // Manejar cuando se alcanza el objetivo.
        // ?? Refactor anti-spam: esta función SOLO modifica el estado
        // (target += originalTarget, hide del container, etc.). NO anima
        // ni guarda al servidor. La animación + el save se hacen UNA SOLA
        // VEZ desde processTikTokEvent al salir del while loop, aunque la
        // meta se haya alcanzado N veces en una ráfaga (current saltó de
        // 0 a 1.36M en un solo evento ? while loop ejecuta 272 iteraciones).
        // Antes cada iteración llamaba saveGoalToServer y emitía animación
        // ? spam masivo al worker + oscilación visual perpetua.
        ...
        // while loop, no por cada iteración.
        ...
        // último save se ejecuta. Evita spam al worker cuando el while
        ...
        async function saveGoalToServer() {
            try {
                // ? ENVIAR SOLO LAS KEYS DE ESTA MÉTRICA (likes), no todo el config
                const partialConfig = {
        ...
        // [log cleaned]
        // Intentar cargar la configuración con retry (hasta 10 intentos, 200ms entre cada uno)
        let loadAttempts = 0;
        ...
        async function initializeWithRetry() {
            ...
            if(success) {
                ...
                console.warn('[Goals Widget] ?? No se pudo cargar la configuración después de', maxAttempts, 'intentos. Usando valores por defecto.');
        ...
        // Variable para rastrear último timestamp
        ...
        // ? El widget ahora se adapta automáticamente usando CSS responsive (width: 100%, height: 100%)
