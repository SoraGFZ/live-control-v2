...
const EVENT_WIDGET_IDS = ['timer', 'auction', 'event-notification', 'roulette', 'poll', 'winlife']

export function EventsSection({ onJump, profile }) {
  const overlayKey = profile?.overlayKey || ''
  const baseUrl = typeof window !== 'undefined' ? window.location.origin : ''
  const eventWidgets = TIKCONTROL_WIDGETS.filter((w) => EVENT_WIDGET_IDS.includes(w.id))

  return (
    <TikControlModuleShell sectionId="events" onJump={onJump}>
      <article className="tc-premium-hero">
        <span className="tc-premium-badge">Eventos del canal</span>
        <h3>Timer, subastas, notificaciones y metas</h3>
        <p>
          Misma pestaña que TikControl: campanas del directo, widgets de utilidad y metas del live.
        </p>
      </article>

      <div className="section-actions-row">
        <button type="button" className="primary-button" onClick={() => onJump('goals')}>
          Metas del live
        </button>
      ...
      <EventLiveControls profile={profile} />

      <div className="tc-widget-gallery-grid tc-widget-gallery-grid--preview">
        {eventWidgets.map((widget) => {
          const url = buildTikcontrolWidgetUrl(widget, { baseUrl, overlayKey })
          return (
            <OverlayWidgetPreviewCard
              key={widget.id}
              widget={widget}
      ...
      <h4 className="section-inline-title">Metas del evento (goals)</h4>
      <div className="tc-widget-gallery-grid tc-widget-gallery-grid--preview">
...
export function AgenciesSection({ onJump, profile }) {
  const username = profile?.tiktokUsername || 'tu_cuenta'

  return (
    <TikControlModuleShell sectionId="agencies" onJump={onJump} hideIntro>
      <div className="tc-premium-hero" style={{ marginBottom: 8 }}>
        <div>
          <span className="tc-premium-badge">Agencias</span>
          <h3>Gestión de Agencia (Local)</h3>
          <p>
            Versión local del panel de agencias de TikControl. Ideal para gestionar múltiples creadores,
            revisar rendimiento de lives y tener un overview centralizado sin depender de la nube.
          </p>
        </div>
      </div>
      ...
      <div className="tc-widgets-stats">
        <div className="metric-card">
          <span className="metric-label">Creadores gestionados</span>
          <strong>1</strong>
        </div>
        <div className="metric-card">
          <span className="metric-label">Lives este mes</span>
          <strong>—</strong>
        </div>
        <div className="metric-card">
      ...
      <div className="surface-card" style={{ padding: '20px 22px' }}>
        <h4 style={{ marginTop: 0, marginBottom: 8 }}>Tu cuenta principal</h4>
        <p style={{ color: '#94a3b8', marginBottom: 16 }}>
          @{username} — Actualmente estás trabajando en modo <strong>Founder Local</strong>.
          Esta sección puede expandirse para agregar más cuentas de creadores.
        </p>

        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          <button className="primary-button" onClick={() => onJump('live-ops')}>
            Ir a TikTok Live
          </button>
          <button className="secondary-button" onClick={() => onJump('account')}>
            Ver Cuenta
          </button>
          <button className="secondary-button" onClick={() => onJump('community')}>
            Ver Comunidad
          </button>
        </div>
      </div>
      ...
      <div className="empty-state-card" style={{ marginTop: 8 }}>
        <h4>¿Quieres más funcionalidad de agencia?</h4>
        <p>
          Podemos agregar gestión multi-cuenta, reportes semanales, asignación de acciones por creador y más.
          Decime qué necesitás y lo construimos.
        </p>
      </div>
    </TikControlModuleShell>
  )
