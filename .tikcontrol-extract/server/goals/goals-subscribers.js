...
let settings = {
            ...
            try {
                ...
                if(data.ok && data.config) {
                    ...
                    if(data.accumulator && (data.accumulator.subscribers > 0 || data.accumulator.isActive)) {
                        ...
                        settings.current = 0;
                        // console.log('[Goals Widget - subscribers] ?? Iniciando con current=0 (acumulador vac�o)');
                    }
        ...
        function animateProgress(targetPercentage) {
            ...
            if(diff < 2) {
                ...
                // console.log('[Goals Widget - subscribers] ? Actualizaci�n INSTANT�NEA a', currentProgress.toFixed(2) + '%');
                return;
            }
        ...
        async function connectToServer() {
            ...
            if (__TC_IS_HTTPS_PAGE) {
                ...
                console.warn('[Goals Widget - subscribers] P�gina servida en HTTPS sin uid: no se puede conectar al servidor local.');
                return;
            }
            ...
            if(typeof io !== 'undefined') {
                ...
                socket.on('connect_error', (error) => {
                    console.error('[Goals Widget - subscribers] ? Error de conexi�n Socket.IO:', error);
                });
                ...
                socket.on('goals:accumulator:update', (data) => {
                    ...
                    try {
                        ...
                        console.error('[Goals Widget - subscribers] ? Error procesando actualizaci�n del acumulador:', e);
                    }
                });
            } else {
                console.error('[Goals Widget - subscribers] ? Socket.IO no disponible despu�s de cargar el script');
            }
        }
        ...
        async function saveGoalToServer() {
            try {
                // ? ENVIAR SOLO LAS KEYS DE ESTA M�TRICA (subscribers), no todo el config
                const partialConfig = {
                ...
                // console.log('[Goals Widget - subscribers] ?? Guardando SOLO esta m�trica (subscribers) al servidor');
