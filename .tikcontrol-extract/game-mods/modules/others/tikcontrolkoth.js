// TikControl King of the Hill - Minecraft Plugin Integration via ServerTap

const { ipcMain } = require('electron');
const fs = require('fs');
const path = require('path');

const GAME_ID = 'tikcontrol-koth';
const GAME_NAME = 'TikControl KOTH';
const IPC_PREFIX = 'tikcontrolkoth';
...
async function loadEffectsFromCloud() {
  ...
  try {
    ...
    if (commands) {
      ...
      effectsLoaded = true;
      console.log(`[${GAME_NAME}] Cargados ${effectsData.effects?.length || 0} efectos desde la nube`);
    }
  } catch (error) {
...
async function sendEffect(effectId, options = {}) {
  ...
  if (effect.parameters && Array.isArray(effect.parameters)) {
    effect.parameters.forEach(param => {
      ...
      const value = options[`param_${paramName}`] ?? options[paramName] ?? param.default ?? '';
      command = command.replace(`{${paramName}}`, value);
    });
  } else if (effect.params && typeof effect.params === 'object') {
  ...
  const nickname = options.nickname || options.username || 'Viewer';
  command = command
    .replace('{duration}', options.duration ?? 0)
    .replace('{quantity}', options.quantity ?? 0)
    .replace('{player}', options.player || '@p')
    .replace('{invoker}', nickname)
