/**
 * Hollow Knight: Silksong - TikControl Integration Module
 * BepInEx mod (HKSS Core) runs HTTP server on port 55001.
...
let mainWindow = null;
const GAME_ID = 'hksilksong';
const STEAM_APP_ID = 1030300;
...
const COMMAND_MAP = {
    ...
    'damage':       { path: '/damage' },
    'kill':         { path: '/kill' },
    'respawn':      { path: '/respawn' },
    'silk':         { path: '/silk' },
    'rosaries':     { path: '/rosaries' },
    // --- Combat ---
    'nail':         { path: '/nail' },
    // --- Chaos / Effects ---
    ...
    'nojump':       { path: '/nojump' },
    'flipcamera':   { path: '/flipcamera' },
    'invincible':   { path: '/invincible' },
    // --- Teleport ---
...
function initialize(window) {
    mainWindow = window;
    registerIpcHandlers();
    console.log('[hksilksong] Modulo inicializado');
}
...
function saveGamePath(gamePath, profileId) {
    ...
    writeConfig(config);
    console.log('[hksilksong] Ruta guardada:', gamePath);
}

function isModInstalled(profileId) {
    const gamePath = getGamePath(profileId);
    if (!gamePath) return false;
    return fs.existsSync(path.join(gamePath, 'BepInEx', 'plugins', 'HollowKnightSillkSongMod.dll'))
...
async function executeCommand(commandId, parameters = {}) {
    ...
    if (!cmd) {
        throw new Error(`[hksilksong] Comando desconocido: ${commandId}`);
    }
    ...
    // Parametro principal: effect (scene para teleport, valor para el resto)
    const mainParam = parameters.effect || parameters.scene || parameters.item;
    if (mainParam !== undefined && mainParam !== null) {
        payload.effect = String(mainParam);
    }

    // Gate opcional para teleport
    if (parameters.gate !== undefined && parameters.gate !== '') {
        payload.gate = String(parameters.gate);
    }

    console.log('[hksilksong] Ejecutando:', endpoint, JSON.stringify(payload), '(id:', commandId + ')');
...
function _sendHttpRequest(endpoint, payload) {
    ...
    console.log('[hksilksong] ->', endpoint, body);
    ...
    return new Promise((resolve, reject) => {
        const req = http.request({
            ...
            res.on('end', () => {
                console.log('[hksilksong] <-', res.statusCode, data, '(', endpoint, ')');
        ...
        req.on('error', (err) => {
            console.error('[hksilksong] Error HTTP:', err.message);
...
function registerIpcHandlers() {
    ipcMain.handle('hksilksong:isConnected', async (event, profileId) => {
        try {
    ...
    ipcMain.handle('hksilksong:executeEffect', async (event, command, parameters = {}) => {
        try {
    ...
    ipcMain.handle('hksilksong:setGamePath', async (event, profileId, gamePath) => {
        try {
            ...
            console.error('[hksilksong] Error configurando ruta:', error);
            throw error;
        }
    });

    ipcMain.handle('hksilksong:getGamePath', async (event, profileId) => {
        const p = getGamePath(profileId);
        return p ? { path: p, success: true } : null;
    });

    ipcMain.handle('hksilksong:checkModStatus', async (event, profileId) => {
        try {
            const gamePath = getGamePath(profileId);
    ...
    ipcMain.handle('hksilksong:installMod', async (event, profileId) => {
        try {
            const gamePath = getGamePath(profileId);
            if (!gamePath) throw new Error('Primero debes configurar la ruta del juego');

            const MOD_URL = 'https://storage.tikcontrol.live/games/hksilksong/mod.zip';
            const tmpZip = path.join(os.tmpdir(), 'tikcontrol-hksilksong-mod.zip');

            console.log('[hksilksong] Descargando mod desde AWS...');
            await _downloadFile(MOD_URL, tmpZip);

            const AdmZip = require('adm-zip');
            const zip = new AdmZip(tmpZip);
            zip.extractAllTo(gamePath, true);
            console.log('[hksilksong] Mod extraido en:', gamePath);
            try { fs.unlinkSync(tmpZip); } catch (_) {}

            return { success: true, message: 'Mod instalado correctamente.', modInstalled: true };
        } catch (error) {
            console.error('[hksilksong] Error instalando mod:', error);
            throw error;
        }
    });

    ipcMain.handle('hksilksong:uninstallMod', async (event, profileId) => {
        try {
            const gamePath = getGamePath(profileId);
            ...
            for (const f of [
                ...
                if (fs.existsSync(f)) { fs.unlinkSync(f); console.log('[hksilksong] Eliminado:', f); }
            }
            const bepinex = path.join(gamePath, 'BepInEx');
            if (fs.existsSync(bepinex)) {
                fs.rmSync(bepinex, { recursive: true });
                console.log('[hksilksong] Carpeta eliminada:', bepinex);
            }

            return { success: true, message: 'Mod desinstalado correctamente' };
        } catch (error) {
            console.error('[hksilksong] Error desinstalando mod:', error);
            throw error;
        }
    });

    ipcMain.handle('hksilksong:launchGame', async () => {
        try {
            const { shell } = require('electron');
            await shell.openExternal(`steam://rungameid/${STEAM_APP_ID}`);
            return { success: true, method: 'steam' };
        } catch (error) {
            console.error('[hksilksong] Error:', error);
            throw error;
        }
    });
}
