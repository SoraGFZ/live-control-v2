// TikControl Diamond Challenge - Minecraft Plugin Integration via ServerTap

const { ipcMain } = require('electron');
const fs = require('fs');
const path = require('path');

const GAME_ID = 'tikcontrol-diamond';
const GAME_NAME = 'Diamond Challenge';
const IPC_PREFIX = 'tikcontroldiamondchallenge';
...
async function getStatus() {
  try {
    ...
    const connected = await new Promise((resolve) => {
      const req = http.get(
        { hostname: 'localhost', port: 4567, path: '/v1/server', timeout: 1500 },
        (res) => resolve(res.statusCode < 500)
      );
      req.on('error', () => resolve(false));
      req.on('timeout', () => { req.destroy(); resolve(false); });
    });
    return { connected, method: 'ServerTap' };
  } catch {
    return { connected: false, method: 'ServerTap' };
  }
}
...
function registerIpcHandlers() {
  ...
  try {
    ipcMain.handle(`${IPC_PREFIX}:installServer`, async (event, serverPath) => {
      try {
        ...
        console.log(`[${GAME_NAME}] Descargando Paper 1.21.1...`);
        await _downloadFile(paperUrl, serverJarPath);

        fs.writeFileSync(eulaPath, 'eula=true\n', 'utf8');

        const startScript = `@echo off\ntitle Diamond Challenge - Minecraft Server\necho Iniciando servidor Diamond Challenge...\njava -Xmx4G -Xms2G -jar server.jar nogui\npause\n`;
  ...
  // Instalar plugin DiamondChallenge
  try {
    ipcMain.handle(`${IPC_PREFIX}:installPlugin`, async (event, serverPath) => {
      try {
        ...
        if (!gameInfo?.mod) return { success: false, error: 'Mod no encontrado en manifest' };
...
function initialize(mainWindow) {
  console.log(`[${GAME_NAME}] Inicializando modulo (modo ServerTap)...`);
  loadEffects();
  registerIpcHandlers();
}
